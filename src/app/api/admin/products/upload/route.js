...
export async function POST(req) {
  try {
    ...
    console.log(`[Product Upload API] Compressing product image and uploading to Supabase Storage...`);
    ...
    if (error) {
      console.error('[Product Upload API] Supabase Storage upload error:', error);
    ...
    console.log(`[Product Upload API] Image uploaded to Supabase CDN: ${publicUrl}`);

    return NextResponse.json({ url: publicUrl });
  } catch (error) {
    console.error('[Product Upload API] Error processing image upload:', error);
